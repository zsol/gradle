...
public class FactoryBasedStructNodeInitializer<T, S extends T> extends StructNodeInitializer<S> {
    ...
    public FactoryBasedStructNodeInitializer(InstanceFactory<T> instanceFactory, ManagedStructBindingStore.ManagedStructBinding<S> bindings) {
        super(bindings);
    ...
    protected void initializePrivateData(MutableModelNode modelNode) {
        NewStructSchema<S> schema = bindings.getPublicSchema();
        ModelType<S> type = schema.getType();
        InstanceFactory.ImplementationInfo<T> implementationInfo = instanceFactory.getManagedSubtypeImplementationInfo(type);
